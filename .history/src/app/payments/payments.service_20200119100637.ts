...
export class PaymentsService {
  ...
  private filterByPaginate(userId: string) {
    // let lastVisible;
    return this.filterByUserId(userId).get().pipe(
      map((documentSnapshots) => {
        return documentSnapshots.docs[documentSnapshots.docs.length - 1];
      })
    );
    // return this.filterByUserId(userId).get().subscribe((documentSnapshots) => {
    //   lastVisible = documentSnapshots.docs[documentSnapshots.docs.length - 1];
    //   console.log('last', lastVisible);
    //   return this.getNext(userId, lastVisible);
    // });
  ...
  private getNext(userId: string) {
    const lastVisible = this.filterByPaginate(userId);
    return this.afs.collection<useClass>(collection, ref => ref
      .where(indexKey, '==', userId)
      .orderBy(orderField, orderBy)
      .startAfter(lastVisible)
  ...
  getByUserIdByLastVisible(userId: string): Observable<useClass[]> {
    return this.fetchData(this.getNext(userId));
  }
