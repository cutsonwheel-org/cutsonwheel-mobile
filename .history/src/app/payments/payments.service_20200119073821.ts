...
export class PaymentsService {
  ...
  private filterByUserId(userId: string, limit: number, lastVisible?: any) {
    // const first = this.afs.collection<useClass>(collection, ref => ref
    //   .orderBy(orderField)
    //   .limit(25)
    // );
    // return first.get().subscribe((documentSnapshots) => {
    ...
    //   const next = this.afs.collection<useClass>(collection, ref => ref
    //     .orderBy(orderField)
    ...
    return col.snapshotChanges().pipe(
        map(actions => {
          return actions.map(a => {
            ...
            data.lastVisible = a.payload.doc.data().length - 1;
            return { id, ...data };
          });
        })
      );
  ...
  getByUserId(userId: string, limit?: number): Observable<useClass[]> {
    return this.fetchData(this.filterByUserId(userId, limit));
  }
