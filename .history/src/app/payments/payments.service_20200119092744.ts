...
export class PaymentsService {
  ...
  getPaginate(userId: string) {
    const first = this.filterByUserId(userId);
    return first.get().subscribe((documentSnapshots) => {
      const lastVisible = documentSnapshots.docs[documentSnapshots.docs.length - 1];
      console.log('last', lastVisible);

      const next = this.afs.collection(collection, ref => ref.orderBy('population')
      .startAfter(lastVisible)
      .limit(25));
    });
  }
  ...
  getByUserIdByLastVisible(userId: string, lastVisible: number): Observable<useClass[]> {
    return this.fetchData(this.filterByUserIdScrolled(userId, lastVisible));
  }
