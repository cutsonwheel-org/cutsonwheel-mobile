...
export class PlacesService {
  ...
  // =======================
  getOffers(limit: number = 1): Observable<Places[]> {
    this.offersCollection = this.afs.collection<Places>('offered-places', ref => ref.limit(limit));
    return this.offers = this.offersCollection.snapshotChanges().pipe(
      map(actions => {
  ...
  getMyOffers() {
    this.offersCollection = this.afs.collection<Places>(
      'offered-places',
      ref => ref
      .where('userId', '==', 'ZrR7yJQpckVpPXPnFkaSkCdWE0f2')
    );
    return this.offers = this.offersCollection.snapshotChanges().pipe(
      map(actions => {
        return actions.map(a => {
