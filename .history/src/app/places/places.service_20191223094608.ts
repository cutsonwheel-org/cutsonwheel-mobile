...
export class PlacesService {
  ...
  private placesObs: Observable<Places[]>;
  private placesCol: AngularFirestoreCollection<Places>;
  ...
  constructor(
    ...
    this.placesCol = this.afs.collection<Places>('offered-places');
    this.placesObs = this.placesCol.snapshotChanges().pipe(
      map(actions => {
  ...
  // =======================
  getBookings(): Observable<Places[]> {
    return this.placesObs;
  }

  getBooking(id: string): Observable<Places> {
    return this.placesCol.doc<Places>(id).valueChanges().pipe(
      take(1),
      map(booking => {
        booking.id = id;
        return booking;
  ...
  insertBooking(booking: Places): Promise<DocumentReference> {
    return this.placesCol.add(booking);
  }

  // updateBooking(booking: Places): Promise<void> {
  //   return this.placesCol.doc(booking.id).update({
  //     name: booking.name,
  //     notes: booking.notes
  //   });
  // }
  ...
  deleteBooking(id: string): Promise<void> {
    return this.placesCol.doc(id).delete();
  }
  // =======================
