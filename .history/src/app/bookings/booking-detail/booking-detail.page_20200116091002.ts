...
export class BookingDetailPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.paramMap.subscribe(paramMap => {
      ...
      this.bookingsService.getOne(paramMap.get('bookingId')).pipe(
        ...
        // this.users.displayName = (this.users.roles.assistant) ? details[1].displayName : details[0].displayName;
        // this.users.photoURL = (this.users.roles.assistant) ? details[1].photoURL : details[0].photoURL;
        // this.users.roles.assistant = this.users.roles.assistant;
        // this.users.roles.client = this.users.roles.client;

        const clientDetail = details[0];
  ...
  private getBooking(bookingId: string) {
    this.bookingSub = this.bookingsService.getOne(bookingId).pipe(
      ...
      mergeMap( booking => {
        const assistant = this.usersService.getUser(booking.assistant.assistantId);
        const client = this.usersService.getUser(booking.userId);
