...
export class NewBookingPage implements OnInit {
  ...
  ngOnInit() {
    const segment = this.onGetSegment();
    this.selectedSegment = segment ? segment : 'location';
    this.locationSelected = JSON.parse(localStorage.getItem('location'));
  ...
  private createPlace(latitude: number, longitude: number) {
    ...
    this.getAddress(latitude, longitude)
      ...
      .subscribe(staticMapImageUrl => {
        ...
        this.onSetLocation(JSON.stringify(pickedLocation));
        this.isLoading = false;
      });
  }
