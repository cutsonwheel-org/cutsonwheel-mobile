...
export class ReviewPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    // get location
    this.locationSub = this.getLocation().subscribe((location) => {
    ...
    this.assistantSub = this.getAssistant().subscribe((assistant) => {
      ...
      this.getUser(this.assistant.assistantId);
    });

    this.scheduleSub = this.getSchedule().subscribe((schedule) => {
  ...
  private getUser(assistantId: string) {
    this.userSub = this.usersServce.getUser(assistantId).subscribe((user) => {
      this.user = user;
    });
  }
  ...
  private getLocation(): Observable<PlaceLocation> {
