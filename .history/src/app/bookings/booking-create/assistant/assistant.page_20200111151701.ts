...
export class AssistantPage implements OnInit {
  ...
  pushToArray(assistantList, obj) {
    const index = assistantList.selectedServices.findIndex((e) => e.id === obj.id);

    if (index === -1) {
      assistantList.selectedServices.push(obj);
    } else {
      assistantList.selectedServices[index] = obj;
    }

    console.log(assistantList.selectedServices);
}
