      ...
      map(booking => {
        ...
        return {
          bookings: booking.map(
            b => {
              const d = new Date(b.schedule.datePicked).getFullYear();
              console.log(d);
              // const newSchedule = new Misc().mergeDateTime(b.schedule.datePicked, b.schedule.timePicked);
              return {
                id: b.id,
                assistant: b.assistant,
                location: b.location,
                schedule: {
                  datePicked: new Date(b.schedule.datePicked).toISOString(),
                  // timePicked: new Date(b.schedule.datePicked + 'T' + b.schedule.timePicked).toISOString()
                },
                status: b.status,
                userId: b.userId
              };
