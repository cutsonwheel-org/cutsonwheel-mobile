...
export class BookingsService {
  ...
  getByClientId(userId: string, status: string): Observable<useClass[]> {
    return this.fetchData(this.filterByClientId(userId, status));
  }

  getByAssistantId(userId: string, status: string) {
    return this.fetchData(this.filterByAssistantId(userId, status));
  }
