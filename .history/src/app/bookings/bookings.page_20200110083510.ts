...
export class BookingsPage implements OnInit {
  ...
  private populateBookings(user: any, status: string) {
    ...
    if (user.roles.assistant) {
      /** assistant */
      loadedBooking = this.getAssistantBookings(user.uid, status); // this.bookingService.getByAssistantId(currenctUser.uid);
    } else {
      /** client */
      loadedBooking = this.getClientBookings(user.uid, status); // this.bookingService.getByClientId(currenctUser.uid);
    }
