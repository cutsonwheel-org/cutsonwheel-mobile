...
export class WalletService {
  ...
  getTotalWallet(userId: string): Observable<any> {
    ...
    //   })
      ...
      return this.afs.collection<any>(collection,
        ...
        .orderBy('paymentDate', 'asc')
          ...
          mergeMap((fullResults: any) => {
            console.log(fullResults.payload);
            return this.afs.doc<any>(fullResults.payload.doc.id).valueChanges().pipe(
              take(1),
              map(data => {
                return data;
              })
      ...
      //   }),
      //   mergeMap((fullResults: any) => {
      //     return this.afs.doc<any>(fullResults.paymentId).valueChanges().pipe(
      //       take(1),
      //       map(data => {
      //         return data;
      //       })
  ...
  public getSingleData(paymentId: string): Observable<any> {
    return this.afs.doc<any>(paymentId).valueChanges().pipe(
      take(1),
      map(data => {
        return data;
      })
