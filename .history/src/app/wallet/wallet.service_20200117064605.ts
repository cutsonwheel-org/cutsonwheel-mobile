...
export class WalletService {
    ...
    //   })
      ...
      map(actions => {
        ...
        mergeMap((fullResults: any) => {
          return fullResults.payments = this.getSingleData(fullResults.paymentId);
        });
      })
    );
  }
  ...
  public getSingleData(paymentId: string): Observable<any> {
    return this.afs.collection<any>('payments', ref => ref.orderBy(orderField, orderBy))
    .doc<any>(paymentId).valueChanges().pipe(
      take(1),
      map(data => {
        return data;
      })
